...
import { getAllExpenses, getAllExpenseMonthKeys } from '../data/store';
...
export default function MonthlyExpenses() {
  ...
  function refresh() {
    setMonthKeys(getAllExpenseMonthKeys());
    ...
    <div style={{ animation: 'slideUp 0.3s ease' }}>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div className="card-header">
          <div>
            ...
            <div className="card-subtitle">Xarajatlar oylar bo'yicha guruhlanadi</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <div className="form-group">
            ...
            <Link
              to={`/expenses/${newYear}/${String(newMonth).padStart(2, '0')}`}
      ...
      {monthKeys.length === 0 ? (
        <div className="empty-state">
          ...
          <div className="empty-title">Hali xarajat yo'q</div>
          <div className="empty-desc">Yuqoridan oy tanlang va xarajatlarni kiriting</div>
        </div>
      ) : (
        years.map((year) => {
            ...
            <div key={year} style={{ marginBottom: 28 }}>
              ...
              <div className="month-grid">
                {yearKeys.map((key) => {
                  ...
                  const records = allExpenses[key] || [];
                  const totalExp = records.reduce((s, r) => s + (parseFloat(r.summa) || 0), 0);

                  return (
                    <Link key={key} to={`/expenses/${yr}/${mn}`} className="month-card">
                      ...
                      <div className="month-stats">
                        <div className="month-stat">
                          <div className="month-stat-label">Jami Xarajat</div>
                          <div className="month-stat-value" style={{ color: '#f87171' }}>{fmt(totalExp)}</div>
                        </div>
                        <div className="month-stat">
                          <div className="month-stat-label">O'rtacha</div>
                          <div className="month-stat-value" style={{ color: '#fcd34d' }}>
                            {records.length > 0 ? fmt(Math.round(totalExp / records.length)) : '0'}
                          </div>
                        </div>
                      </div>
                    </Link>
